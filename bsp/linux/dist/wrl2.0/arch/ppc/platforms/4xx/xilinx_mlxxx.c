 ...
 *
 * Xilinx ML300, ML403, ML405, ML410 and ML507 evaluation boards initialization
 *
...
/*
 * I'm noting that xilinx_ml300.[ch]/xilinx_ml403.[ch] files don't appear
 * to be used. Perhaps these will be used in the future bsp.
 */
...
mlxxx_early_serial_map(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(old_ports); i++ ) {
		...
		/* port.membase = ioremap((phys_addr_t)(old_ports[i].iomem_base), 16); */ 	/* original, removed by ecm on 080630 */
		port.membase = ioremap(((phys_addr_t)(old_ports[i].iomem_base)&0xfffff00f), 8);
