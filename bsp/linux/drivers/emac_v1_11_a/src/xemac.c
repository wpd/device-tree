/* $Id: xemac.c,v 1.1 2008/08/14 19:08:45 meinelte Exp $ */
/******************************************************************************
*
...
*                     XEmac_mPhyReset macro in xemac_hw.h.
...
*                     Added the Chesksum offload initialization to control word.
* 1.11a wgr  03/22/07 Converted to new coding style.
* 1.11a rpm  05/23/07 Moved LookupConfig to _sinit.c
* </pre>
******************************************************************************/
...
/************************** Function Prototypes ******************************/
int Initialize(XEmac * InstancePtr, XEmac_Config * ConfigPtr);
			...
			u32 VirtualAddress)
{
	...
	return (Initialize(InstancePtr, ConfigPtr));
}
...
int XEmac_Start(XEmac * InstancePtr)
{
	...
	if (!InstancePtr->IsPolled) {
		...
		if (XEmac_mIsSgDma(InstancePtr)) {
			...
			XEMAC_WRITE_DIER(InstancePtr->BaseAddress,
					      ...
					      XEMAC_ERROR_MASK);
			XEMAC_WRITE_IIER(InstancePtr->BaseAddress,
					      XEM_EIR_DFT_SG_MASK);
			...
			if (InstancePtr->IsSgEndDisable) {
				/* Don't enable SGEND */
				ControlReg &= ~XDC_IXR_SG_END_MASK;
			}
		...
		XEMAC_GINTR_ENABLE(InstancePtr->BaseAddress);
	}
	...
	/*
	 * Enable the transmitter, and receiver (do a read/modify/write to
	 * preserve current settings). There is no critical section here since
	 * this register is not modified during interrupt context.
	 */
	 ...
	 * DMA channels in case there are buffers ready to be sent or received
	 * into.  The DMA SgStart function uses data that can be modified
	 * during interrupt context, so a critical section is required here.
	 */
	if ((XEmac_mIsSgDma(InstancePtr)) && (!InstancePtr->IsPolled)) {
		XEMAC_GINTR_DISABLE(InstancePtr->BaseAddress);

		/*
		 * The only error we care about is if the list has not yet been
		 * created, or on receive, if no buffer descriptors have been
		 * added yet (the list is empty). Other errors are benign at
		 * this point.
		 */
...
int XEmac_Stop(XEmac * InstancePtr)
{
	 ...
	 * here between SgStart and SgStop, and SgStart can be called in
	 * interrupt context, so disable interrupts while calling SgStop.
	 */
	if (XEmac_mIsSgDma(InstancePtr)) {
...
/**
*
* Reset the Ethernet MAC. This is a graceful reset in that the device is
* stopped first. Resets the DMA channels, the FIFOs, the transmitter, and the
* receiver.  The PHY is not reset. Any frames in the scatter-gather descriptor
* lists will remain in the lists. The side effect of doing this is that after a
* reset and following a restart of the device, frames that were in the list
* before the reset may be transmitted or received. Reset must only be called
...
void XEmac_Reset(XEmac * InstancePtr)
{
	 ...
	 * hardware block separately, the reset should occur in the direction
	 * of data flow. For example, for the send direction the reset order is
	 * DMA first, then FIFO, then the MAC transmitter.
	 */
	XEMAC_RESET(InstancePtr->BaseAddress);

	if (XEmac_mIsSgDma(InstancePtr)) {
		/*
		 * After reset, configure the scatter-gather DMA packet
		 * threshold and packet wait bound registers to default values.
		 * Ignore the return values of these functions since they only
		 * return error if the device is not stopped.
		 */
...
static int ConfigureDma(XEmac * InstancePtr)
{
	 ...
	 * scatter-gather DMA is the only possible configuration. Descriptor
	 * space will need to be set later by the upper layer.
	 */
...
*
* @param InstancePtr is a pointer to the XEmac instance to be worked on.
* @param ConfigPtr is a pointer to a config table for the particular device this
...
int Initialize(XEmac * InstancePtr, XEmac_Config * ConfigPtr)
{
	...
	/*
	 * Lookup the device configuration in the temporary CROM table. Use
	 * this configuration info down below when initializing this component.
	 */
	InstancePtr->Config = *ConfigPtr;
	...
	/*
	 * If the device is configured for DMA, configure the send and receive
	 * DMA channels in the MAC.
	 */
	if (XEmac_mIsDma(InstancePtr)) {
		 ...
		 * BD that uses it. It is not applicable to all data types so
		 * the driver needs to handle each call individually
		 */
	 ...
	 * before we reset the device and the PHY below, which may seem a bit
	 * odd. The choice was made to move it here rather than remove the
	 * asserts in various functions (e.g., Reset() and all functions that
	 * it calls).  Applications that use multiple threads, one to
	 * initialize the XEmac driver and one waiting on the IsReady condition
	 * could have a problem with this sequence.
	 */
