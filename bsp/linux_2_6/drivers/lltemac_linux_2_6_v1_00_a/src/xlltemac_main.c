...
#define MARVELL_88E1111_PHY_SPECIFIC_CONTROL_REG_OFFSET 16
	_XLlTemac_PhyWrite(&lp->Emac, lp->gmii_addr, MARVELL_88E1111_PHY_SPECIFIC_CONTROL_REG_OFFSET, 0xF078);
#endif
	...
	if (phy_carrier != netif_carrier) {
		if (phy_carrier) {
			...
			netif_carrier_on(dev);
                        /* Set up speed according to negotiated speed */
                        set_mac_speed(lp);
		}
		else {
...
static irqreturn_t xenet_temac_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	struct net_device *dev = dev_id;
	struct net_local *lp = (struct net_local *) dev->priv;

	/*
	 * If we get here, mostly what we care about here is the RxRject
	 * interrupts. Explanation below:
	 *
...
static irqreturn_t xenet_fifo_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	struct net_device *dev = dev_id;
	struct net_local *lp = (struct net_local *) dev->priv;
	u32 irq_status;
...
static irqreturn_t xenet_dma_rx_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	u32 irq_status;
	struct net_device *dev = dev_id;
	struct net_local *lp = (struct net_local *) dev->priv;
...
static irqreturn_t xenet_dma_tx_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	u32 irq_status;
	struct net_device *dev = dev_id;
	struct net_local *lp = (struct net_local *) dev->priv;
...
static int xenet_open(struct net_device *dev)
{
	...
	irqval = request_irq(dev->irq, &xenet_temac_interrupt, SA_INTERRUPT, dev->name, dev);
	if (irqval) {
		printk(KERN_ERR
 ...
 */
static int
xenet_DmaSend_internal(struct sk_buff *skb, struct net_device *dev)
{
	struct net_local *lp;
	XLlDma_Bd *bd_ptr;
	int result;
	 ...
	 * if kernel >= 2.6.19
	 * -------------------
	 * The CHECKSUM_HW value has long been used in the networking subsystem
	 * to support hardware checksumming. That value has been replaced with
	 * CHECKSUM_PARTIAL (intended for outgoing packets where the job must
	 * be completed by the hardware) and CHECKSUM_COMPLETE (for incoming
	 * packets which have been completely checksummed by the hardware).
	 *
	 */
	...
	if (first_bd_ptr == last_bd_ptr) {
		XLlDma_mBdSetStsCtrl(last_bd_ptr, XLLDMA_BD_STSCTRL_SOP_MASK | XLLDMA_BD_STSCTRL_EOP_MASK);
	} else {
...
static void _xenet_DmaSetupRecvBuffers(struct net_device *dev)
{
	...
	while (new_skb) {
		...
		XLlDma_mBdSetStsCtrl(BdCurPtr, XLLDMA_BD_STSCTRL_SOP_MASK | XLLDMA_BD_STSCTRL_EOP_MASK);
...
void free_descriptor_skb(struct net_device *dev)
{
	...
	for (i = 0; i < XTE_RECV_BD_CNT; i++) {
		...
		if (skb) {
			skb_dma_addr =
				(dma_addr_t) XLlDma_mBdGetBufAddr(BdPtr);
...
static int xenet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCDEVPRIVATE + 6:	/* get WAITBOUND */
		...
		XLlDma_BdRingGetCoalesce(RingPtr, &threshold, (u32 *) &(wbnd_arg.waitbound));
...
static int detect_phy(struct net_local *lp, char *dev_name)
{
	...
	for (phy_addr = 31; phy_addr > 0; phy_addr--) {
		...
		if ((phy_reg != 0xFFFF) &&
			...
			printk(KERN_INFO "XLlTemac: PHY detected at address %d.\n",
					phy_addr);
			return phy_addr;
		}
	}
	...
	printk(KERN_WARNING
		"XLlTemac: No PHY detected.  Assuming a PHY at address 0\n");
