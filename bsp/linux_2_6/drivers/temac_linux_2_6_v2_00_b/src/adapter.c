...
free_descriptor_skb (struct net_device *dev)
{
    ...
    for (i=0; i<XTE_RECV_BD_CNT; i++) {
        ...
        if (skb) {
            skb_dma_addr = (dma_addr_t)XDmaBdV3_mGetBufAddrLow(BdPtr);
...
xenet_ethtool_get_settings (struct net_device *dev, struct ethtool_cmd* ecmd)
{
    ...
    if (XTemac_mIsSgDma(&lp->Emac)) {
        /* get TX threshold */
        if ((ret = XTemac_IntrSgCoalGet(&lp->Emac, XTE_SEND, &threshold, &timer))
                == XST_SUCCESS) {
        ...
        /* get RX threshold */
        if ((ret = XTemac_IntrSgCoalGet(&lp->Emac, XTE_RECV, &threshold, &timer))
                == XST_SUCCESS) {
...
xenet_ethtool_set_settings (struct net_device *dev, struct ethtool_cmd* ecmd)
{
    ...
    if ((ecmd->duplex != DUPLEX_FULL) ||
            (ecmd->transceiver != XCVR_INTERNAL) ||
            (ecmd->phy_address && (ecmd->phy_address != lp->Emac.BaseAddress))) {
        return -EOPNOTSUPP;
    }

    if  ((ecmd->speed != 1000) && (ecmd->speed != 100) && (ecmd->speed != 10)) {
        printk(KERN_ERR
...
xenet_ethtool_get_coalesce (struct net_device *dev, struct ethtool_coalesce* ec)
{
    ...
    if ((ret = XTemac_IntrSgCoalGet(&lp->Emac, XTE_RECV, &threshold, &waitbound))
                                        != XST_SUCCESS) {
    ...
    if ((ret = XTemac_IntrSgCoalGet(&lp->Emac, XTE_SEND, &threshold, &waitbound))
                                        != XST_SUCCESS) {
        printk(KERN_ERR "%s: XTemac: IntrSgCoalGet error %d\n", dev->name, ret);
        return -EIO;
    }
...
void disp_bd_ring(XDmaV3_BdRing bd_ring) {
    ...
    for (idx = 0; idx < num_bds; idx++) {
        printk("%3d %08x %08x %08x %08x %08x %08x %08x %08x %08x\n",
            idx, cur_bd_ptr, cur_bd_ptr[XDMAV3_BD_DMASR_OFFSET / sizeof(*cur_bd_ptr)],
            cur_bd_ptr[XDMAV3_BD_LSBA_OFFSET / sizeof(*cur_bd_ptr)],
            cur_bd_ptr[XDMAV3_BD_BDA_OFFSET / sizeof(*cur_bd_ptr)],
            cur_bd_ptr[XDMAV3_BD_LENGTH_OFFSET / sizeof(*cur_bd_ptr)],
...
xenet_ethtool_get_regs (struct net_device *dev, struct ethtool_regs* regs, void* ret)
{
    ...
    for (i=0; i<EMAC_REGS_N; i++) {
        if ((r = XTemac_PhyRead (&(lp->Emac), lp->gmii_addr, i, &(dump->data[i])))
                != XST_SUCCESS) {
...
xenet_do_ethtool_ioctl (struct net_device *dev, struct ifreq *rq)
{
    ...
    case ETHTOOL_GRXCSUM: { /* Get rx csum offload info. Use "-k" w/ ethtool */
        ...
        edata.data = (lp->local_features & LOCAL_FEATURE_RX_CSUM) != 0;
    ...
    case ETHTOOL_GSTRINGS: {
        ...
        if (copy_to_user(addr, strings, gstrings.len * ETH_GSTRING_LEN)) {
            return -EFAULT;
        }
        ret = 0;
        break;
    ...
    case ETHTOOL_GSTATS: {
        ...
        } stats = { {ETHTOOL_GSTATS, XENET_STATS_LEN} };
...
xenet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    case SIOCDEVPRIVATE + 3:        /* set THRESHOLD */
        ...
        if ((ret = XTemac_IntrSgCoalSet(&lp->Emac, thr_arg.direction,
                                        thr_arg.threshold, timer)) != XST_SUCCESS){
            spin_unlock_irqrestore(&XTE_spinlock, flags);
            return -EIO;
        }
        if (dev_started == TRUE) {
    ...
    case SIOCDEVPRIVATE + 4:        /* set WAITBOUND */
        ...
        if ((ret = XTemac_IntrSgCoalSet(&lp->Emac, wbnd_arg.direction, threshold,
    ...
    case SIOCDEVPRIVATE + 5:        /* get THRESHOLD */
        ...
        if ((ret = XTemac_IntrSgCoalGet(&lp->Emac, thr_arg.direction,
                        (u16 *)&(thr_arg.threshold), &timer)) != XST_SUCCESS){
            return -EIO;
        }
    ...
    case SIOCDEVPRIVATE + 6:        /* get WAITBOUND */
        ...
        if ((ret = XTemac_IntrSgCoalGet(&lp->Emac, wbnd_arg.direction,
                        &threshold, (u16 *)&(wbnd_arg.waitbound))) != XST_SUCCESS){
            return -EIO;
        }
...
static int xtenet_probe(struct device *dev)
{
    ...
    /* param check */
    if (!pdev) {
        printk(KERN_ERR "XTemac: Internal error. Probe called with NULL param.\n");
        rc = -ENODEV;
        goto error;
    }
...
//    Config.Dre             = pdata->dre;
    ...
    if (XTemac_mIsSgDma(&lp->Emac)) {
        ...
        XTemac_SetHandler(&lp->Emac, XTE_HANDLER_SGSEND, SgSendHandler, lp);
        XTemac_SetHandler(&lp->Emac, XTE_HANDLER_SGRECV, SgRecvHandler, lp);
        ...
        if ((xs = XTemac_IntrSgCoalSet(&lp->Emac, XTE_SEND, DFT_TX_THRESHOLD,
                            DFT_TX_WAITBOUND)) != XST_SUCCESS) {
            printk(KERN_ERR
                   "XTemac: could not set SEND pkt threshold/waitbound, ERROR %d", xs);
        }
        if ((xs = XTemac_IntrSgCoalSet(&lp->Emac, XTE_RECV, DFT_RX_THRESHOLD,
                            DFT_RX_WAITBOUND)) != XST_SUCCESS){
            printk(KERN_ERR
                   "XTemac: Could not set RECV pkt threshold/waitbound ERROR %d", xs);
        }
    } else {
        printk(KERN_INFO "XTemac: using FIFO direct interrupt driven mode.\n");
        XTemac_SetHandler(&lp->Emac, XTE_HANDLER_FIFORECV, FifoRecvHandler, ndev);
        XTemac_SetHandler(&lp->Emac, XTE_HANDLER_FIFOSEND, FifoSendHandler, ndev);
...
#if ! XTE_AUTOSTRIPPING
    lp->stripping = (XTemac_GetOptions (&(lp->Emac)) & XTE_FCS_STRIP_OPTION) != 0;
#endif
    ...
    printk(KERN_INFO
           ...
           ((hwid >> 16) & 0x1f) + 'a', (hwid >> 16) & 0xff, (hwid >> 0) & 0xff);
