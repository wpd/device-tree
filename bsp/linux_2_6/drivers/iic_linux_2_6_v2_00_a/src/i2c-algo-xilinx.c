...
module_param(scan, int, 0);
MODULE_PARM_DESC(scan, "Scan for active chips on the bus");

#ifndef I2C_DRIVERID_EXP0 
#define I2C_DRIVERID_EXP0 0xF0  /* experimental use ids */
#endif
...
#else
	dev = kzalloc(sizeof(struct xiic_data), GFP_KERNEL);
#endif
	if (!dev) {
