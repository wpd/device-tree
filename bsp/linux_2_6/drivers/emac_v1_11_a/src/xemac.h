/* $Id: xemac.h,v 1.1 2008/08/14 19:08:45 meinelte Exp $ */
/******************************************************************************
*
...
*   - Transmit and Receive checksum offloading support
*
...
* taken care of by the Xilinx driver software if it exists for that RTOS.
*
...
* <br><br>
* If the EMAC device has checksum offloading feature turned on, direct FIFO
* and polled mode functions in this driver do NOT work. This should not be
* any problem because checksum offloading feature depends on scatter-gather
* DMA and scatter-gather DMA mode functions will be chosen automatically
* by this driver if scatter-gather DMA is included in hardware.
*
* <pre>
...
*                     functions. Moved some IFG and PHY constants to xemac_hw.h.
* 1.00f rmm  10/19/04 Added programmable CAM address filtering. Added jumbo
*                     frame support. Added XEmac_PhyReset() function.
* 1.01a ecm  09/01/05 Added DRE and Checksum Offload support through Control
*                     words in instance which are set at initialization.
...
#include "xstatus.h"
#include "xpacket_fifo_v2_10_a.h"	/* Uses v2.00a of Packet Fifo */
...
typedef struct {
	...
	int HasCounters;   /**< Does device have counters? */
	u8 IpIfDmaConfig;   /**< IPIF/DMA hardware configuration */
	int HasMii;	   /**< Does device support MII? */
	int HasCam;	   /**< Does device have multicast CAM */
	int HasJumbo;	   /**< Can device transfer jumbo frames */
...
typedef struct {
	...
	u32 PhysAddress;	/* Physical Base address */
	int IsStarted;		/* Device is currently started */
	...
	int IsSgEndDisable;	/* Does SG DMA enable SGEND interrupt */
...
#define XEmac_mHasCam(InstancePtr) \
    (((InstancePtr)->Config.HasCam == 1) ? TRUE : FALSE)
...
#define XEmac_mHasJumbo(InstancePtr) \
    (((InstancePtr)->Config.HasJumbo == 1) ? TRUE : FALSE)
...
int XEmac_VmInitialize(XEmac * InstancePtr, u16 DeviceId, u32 VirtualAddress);
...
int XEmac_SetSgRecvSpace(XEmac * InstancePtr, u32 *MemoryPtr, u32 ByteCount, void *PhyPtr);
int XEmac_SetSgSendSpace(XEmac * InstancePtr, u32 *MemoryPtr, u32 ByteCount, void *PhyPtr);
