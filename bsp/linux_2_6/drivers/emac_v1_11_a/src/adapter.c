        ...
        while (!(XDmaChannel_IsSgListEmpty(&(lp->Emac.SendChannel)))) { /* list isn't empty, has to be cleared */
            ...
            pci_unmap_single(NULL,
                (u32) XBufDescriptor_GetSrcAddress(BdPtr),
                 XBufDescriptor_GetLength(BdPtr), DMA_TO_DEVICE);
            lp->stats.tx_errors++;
        }
...
SgRecvHandlerBH (unsigned long p)
{
    ...
    while (1) {
        ...
        while (NumBds != 0) {
            ...
            new_skb_vaddr = (u32) dma_map_single(NULL, new_skb->data,
                             XEM_MAX_FRAME_SIZE,
                             DMA_FROM_DEVICE);
...
free_descriptor_skb (struct net_device *dev)
{
    ...
    for (i=0; i<XEM_DFT_RECV_DESC; i++) {
        skb = (struct sk_buff*)XBufDescriptor_GetId(BdPtr);
        pci_unmap_single(NULL, virt_to_bus(skb->data), XBufDescriptor_GetLength(BdPtr), DMA_FROM_DEVICE);
        dev_kfree_skb(skb);
...
static int xenet_probe(struct device *dev)
{
    ...
    /* param check */
    if (!pdev) {
        printk(KERN_ERR "XEmac: Internal error. Probe called with NULL param.\n");
        rc = -ENODEV;
        goto error;
    }
    ...
    if (XEmac_mIsSgDma(&lp->Emac)) {
        ...
        if (XEmac_mIsTxDre(&lp->Emac) == TRUE) {
            ...
            if (XEmac_mIsTxHwCsum(&lp->Emac)) {
                printk(KERN_ERR "XEmac: HW CONFIGURATION ERROR, "
        ...
        if (XEmac_mIsTxHwCsum(&lp->Emac)) {
            printk(KERN_ERR "XEmac: TX Checksum offload Mode enabled.\n");
        }

        if (XEmac_mIsRxHwCsum(&lp->Emac)) {
            printk(KERN_ERR "XEmac: RX Checksum offload Mode enabled.\n");
        }
    ...
    for (phy_addr = 0; phy_addr < 31; phy_addr++) {
        ...
        for (repeat = 0; repeat < 4; repeat++) {
            ...
            Result = XEmac_PhyRead(&lp->Emac, phy_addr, MII_PHYSID1, &check_reg);
        ...
        if (4 == repeat) {
             ...
             * bad is happening.
             */
            Result = XEmac_PhyRead(&lp->Emac, phy_addr, MII_PHYSID2, &rev);
            if (XST_SUCCESS != Result) {
                printk("XEmac: Error reading PHY revision for PHY at address %d! "
