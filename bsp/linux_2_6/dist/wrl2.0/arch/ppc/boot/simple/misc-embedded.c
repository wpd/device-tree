...
#if defined(CONFIG_XILINX_ML410) && defined(CONFIG_PCI)
...
load_kernel(unsigned long load_addr, int num_words, unsigned long cksum, bd_t *bp)
{
	 ...
	 */
	embed_config(&bp);
#if defined(CONFIG_SERIAL_CPM_CONSOLE) || defined(CONFIG_SERIAL_8250_CONSOLE) || defined(CONFIG_XILINX_UARTLITE_CONSOLE)
