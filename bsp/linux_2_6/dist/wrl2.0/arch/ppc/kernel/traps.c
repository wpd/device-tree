...
#ifdef CONFIG_XILINX_FPU_SIIE_WORKAROUND
u16 excep_state = 0;
#endif
...
void _exception(int signr, struct pt_regs *regs, int code, unsigned long addr)
{
	...
	MARK(kernel_arch_trap_entry, "%ld %ld", regs->trap,
...
#ifdef CONFIG_TSI108_BRIDGE_PPC
...
#endif /* CONFIG_TSI108_BRIDGE_PPC */
	return 0;
}
...
#ifdef CONFIG_XILINX_FPU_SIIE_WORKAROUND
	/*
	 * Spurious interrupt, for no reason?
	 */
	if (! (reason & (REASON_ILLEGAL | REASON_FP | REASON_TRAP | REASON_PRIVILEGED))) {
		return;
	}
	/*
	 * Xilinx FPU spurious illegal instruction exception, apparently valid by msr bits?
	 */
	if (excep_state == 0) {
		excep_state = 1;
		return;
