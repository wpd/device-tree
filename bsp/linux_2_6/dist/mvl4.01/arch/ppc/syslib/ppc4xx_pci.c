...
extern int ppc4xx_map_irq(struct pci_dev *dev, unsigned char idsel,
			  unsigned char pin);
...
ppc4xx_find_bridges(void)
{
...
#if defined(CONFIG_XILINX_VIRTEX) && defined(CONFIG_PCI)
...
#endif /* defined(CONFIG_XILINX_VIRTEX) && defined(CONFIG_PCI) */
	...
	ppc_md.pci_map_irq = ppc4xx_map_irq;
}
