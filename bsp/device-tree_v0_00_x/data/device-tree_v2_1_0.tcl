	...
	switch -exact $type {
		...
		"axi_dma" {
			...
			if {$tx_chan == 1} {
				...
				set txchanname [format_name [format "%s@%x" $namestring $baseaddr]]

				set txchan {}
				lappend txchan [list compatible stringtuple [list "xlnx,axi-dma-mm2s-channel"]]
				set tmp [scan_int_parameter_value $slave "C_INCLUDE_MM2S_DRE"]
				lappend txchan [list "xlnx,include-dre" hexint $tmp]

				set tmp [xget_hw_parameter_handle $slave "C_M_AXIS_MM2S_TDATA_WIDTH"]

				if {$tmp != ""} {
					set tmp [scan_int_parameter_value $slave "C_M_AXIS_MM2S_TDATA_WIDTH"]
					lappend txchan [list "xlnx,datawidth" hexint $tmp]
				}

				set tmp [xget_hw_parameter_handle $slave "C_M_AXIS_MM2S_DATA_WIDTH"]
				if {$tmp != ""} {
					set tmp [scan_int_parameter_value $slave "C_M_AXIS_MM2S_DATA_WIDTH"]
					lappend txchan [list "xlnx,datawidth" hexint $tmp]
				}

				set chantree [list $txchanname tree $txchan]
				set chantree [gen_interrupt_property $chantree $slave $intc [list "mm2s_introut"]]
			...
			if {$rx_chan == 1} {
				set namestring "dma-channel"
				set rxchanname [format_name [format "%s@%x" $namestring [expr $baseaddr + 0x30]]]

				set rxchan {}
				lappend rxchan [list compatible stringtuple [list "xlnx,axi-dma-s2mm-channel"]]
				set tmp [scan_int_parameter_value $slave "C_INCLUDE_S2MM_DRE"]
				lappend rxchan [list "xlnx,include-dre" hexint $tmp]	

				set tmp [xget_hw_parameter_handle $slave "C_S_AXIS_S2MM_TDATA_WIDTH"]

				if {$tmp != ""} {
					set tmp [scan_int_parameter_value $slave "C_S_AXIS_S2MM_TDATA_WIDTH"]
					lappend rxchan [list "xlnx,datawidth" hexint $tmp]
				}

				set tmp [xget_hw_parameter_handle $slave "C_S_AXIS_S2MM_DATA_WIDTH"]
				if {$tmp != ""} {
					set tmp [scan_int_parameter_value $slave "C_S_AXIS_S2MM_DATA_WIDTH"]
					lappend rxchan [list "xlnx,datawidth" hexint $tmp]
				}

				set chantree [list $rxchanname tree $rxchan]
				set chantree [gen_interrupt_property $chantree $slave $intc [list "s2mm_introut"]]
		...
		"axi_vdma" {
			...
			if {$tx_chan == 1} {
				...
				set txchan {}
				lappend txchan [list compatible stringtuple [list "xlnx,axi-vdma-mm2s-channel"]]
				set tmp [scan_int_parameter_value $slave "C_INCLUDE_MM2S_DRE"]
				lappend txchan [list "xlnx,include-dre" hexint $tmp]

				set tmp [scan_int_parameter_value $slave "C_MM2S_GENLOCK_MODE"]
				lappend txchan [list "xlnx,genlock-mode" hexint $tmp]
			...
			if {$rx_chan == 1} {
				...
				set rxchan {}
				lappend rxchan [list compatible stringtuple [list "xlnx,axi-vdma-s2mm-channel"]]
				set tmp [scan_int_parameter_value $slave "C_INCLUDE_S2MM_DRE"]
				lappend rxchan [list "xlnx,include-dre" hexint $tmp]	

				set tmp [scan_int_parameter_value $slave "C_S2MM_GENLOCK_MODE"]
				lappend rxchan [list "xlnx,genlock-mode" hexint $tmp]
